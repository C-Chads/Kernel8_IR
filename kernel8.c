...
void is_prime(state3* c){
	state3 val = (*c);
	if(val.u == 2 || val.u == 3) return;
	if(val.u == 1) 
		{*c = state3_zero(); return;}
	if(kb_mod_s3(statemix3(val, to_state3(2))).state3s[0].u == 0) 
		{*c = state3_zero(); return;}
	for(uint32_t i = 3; kb_mul_s3( statedup3(to_state3(i)) ).state3s[0].u < val.u; i+=2){
...
K8_SHARED_STATE_WIND(big_shared_process20, //new name
...
void k_dupe_upper4(state4 *c){
	*state_ptr_low4(c) = *state_ptr_high4(c);
}
...
//if you simply use K8_MULTIPLEX then you will get "medium-high" multithreaded code.
...
K8_MULTIPLEX_PARTIAL_ALIAS(name, func, nn, nm, start, end, iscopy, alias)
K8_MULTIPLEX_INDEXED_PARTIAL_ALIAS(name, func, nn, nnn, nm, start, end, iscopy, alias)
K8_SHUFFLE_IND32_PARTIAL(name, func, nn, nm, start, end, iscopy)
K8_SHUFFLE_IND16_PARTIAL(name, func, nn, nm, start, end, iscopy)
K8_SHUFFLE_IND8_PARTIAL(name, func, nn, nm, start, end, iscopy)
K8_MULTIPLEX_INDEXED_EMPLACE_PARTIAL(name, func, nn, nnn, nm, start, end, iscopy)
...
K8_SHARED_STATE_PARTIAL_WIND(name, func, nn, nnn, nm, start, end, sharedind, nwind, whereind, doind, iscopy)
K8_RO_SHARED_STATE_PARTIAL_ALIAS_WIND(name, func, nn, nnn, nm, start, end, sharedind, nwind, whereind, doind, iscopy, alias)
K8_MULTIPLEX_HALVES_PARTIAL_ALIAS(name, func, nn, nnn, nm, start, end, iscopy, alias)
K8_MULTIPLEX_MULTIK8_PARTIAL_ALIAS(name, funcarr, nn, nm, start, end, iscopy, alias)
//Nlogn functionality, an "i,j" nested loop
//i = start; i < end - 1; i++
//j = i+1; j < end; j++
K8_MULTIPLEX_NLOGN_PARTIAL(name, func, nn, nnn, nm, start, end, iscopy)
K8_MULTIPLEX_NLOGNRO_PARTIAL_ALIAS(name, func, nn, nnn, nm, start, end, iscopy, alias)
...
K8_MULTIPLEX_DATA_EXTRACTION_PARTIAL_ALIAS(name, func, nproc, nn, nm, start, end, iscopy, alias)
*/
//Generate a multiplexing of and127 from state1 to state3.
//Notice the SIMD parallelism hint,
K8_MULTIPLEX_SIMD(and127_mt3, //New kernel's name.
...
K8_MULTIPLEX_SIMD(k_endian_cond_swap3_simd_mtp20, k_endian_cond_byteswap3, 3, 20, 0)
...
K8_MULTIPLEX_INDEXED_NP(fk_printer8ind_np_mtpi3, fk_printer8ind, 1, 2, 3, 0);
K8_MULTIPLEX_INDEXED_NP(fk_printer8ind_np_mtpi20, fk_printer8ind, 1, 2, 20, 0);
K8_MULTIPLEX_INDEXED_NP(fk_printer8ind_np_mtpi30, fk_printer8ind, 1, 2, 30, 0);
//Print as uint8_t's with 32 bit index.
K8_MULTIPLEX_INDEXED_NP(fk_printer8ind32_np_mtpi20, fk_printer8ind32, 3, 4, 20, 0);
K8_MULTIPLEX_INDEXED_NP(fk_printer8ind32_np_mtpi30, fk_printer8ind32, 3, 4, 30, 0);
//Emplacing modsort
//mtie stands for "multiplex indexed emplace"
//mtpie stands for "multiplex pointer indexed emplace"
K8_MULTIPLEX_INDEXED_EMPLACE(k_modsort_mtpie20, k_modsort, 3, 4, 20, 0);
//Shuffle.
K8_SHUFFLE_IND32(k_shuffler1_3_20, k_add1_3, 3, 20, 0)
//Shared state worker.
//sharedp stands for "shared pointer". it is a pass-by-pointer shared-type algorithm kernel
K8_SHARED_STATE(k_sum32_sharedp3_20, k_sum32, 3, 4, 20, 0)

//K8_MULTIPLEX_HALVES(name, func, nn, nnn, nm, iscopy)
...
Argument 1: 
*/
K8_MULTIPLEX_NLOGN(k_incrementhalves4_nlognp20, k_incrementhalves4, 3, 4, 20, 0)


//Nlong
K8_MULTIPLEX_NLOGNRO(k_upper3_4_increment_nlognrop20, k_upper3_4_increment, 3, 4, 20, 0)
//K8_MULTIPLEX_NLOGNRO(k_upper3_4_increment_nlognro20, k_upper3_4_increment, 3, 4, 20, 1);
//K8_MULTIPLEX_NLOGN(k_incrementhalves4_nlogn20, k_incrementhalves4, 3, 4, 20, 1);
//The real magic! We can use our previously generated kernels to
//create NEW kernels.
//This has *infinite possibilities*.
K8_MULTIPLEX_SUPARA(k_dupe_upper4_sharedp3_20_mtp30,k_dupe_upper4_sharedp3_20, 20, 30,0)
...
*/
K8_MULTIPLEX_DATA_EXTRACTION_NP(fk_byteprinter_extract3, //new kernel name
									fk_printer, //old
...
//K8_MULTIPLEX_DATA_EXTRACTION_SUPARA(fk_byteprinter_extract3_supara, fk_printer, 2, 3, 20, 0)
